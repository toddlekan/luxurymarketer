...
class Magento_Template_Helper{
	 ...
	 * Prints the given attribute, will parse in other attributes designated between {} by default
	 */
	public function product_attribute($attribute = 'name', $parse = true){
		if($this->inside_product_loop()){
			
			$return = $this->magento_products[$this->i]['result'][$attribute];
			...
			if($parse){
				...
				if (preg_match_all('/{([^}]*)}/', $return, $matches)) {
					...
					foreach($matches[1] as $val){
						$return = str_replace("{".$val."}", $this->magento_products[$this->i]['result'][$val], $return);
					}
				}
			}
	...
	/**
	 * Added by Todd Lekan www.toddlekan.com 12-10-2012
	 * Prints the magento_products array as a base64 encoded serialized object
	 */
	public function products_base64_serialize(){
		if($this->inside_product_loop()){
		
			return base64_encode(serialize($this->magento_products));
	 ...
	 * Submits an order
	 */
	public function submit_order($submission){
		
		$magento_class = new Magento();
		return $magento_class->submitOrder($submission);
...
* Added by Todd Lekan www.toddlekan.com 12-10-2012
*/
function magento_product_attribute($attribute = 'name', $parse = true){
	global $Magento;
	echo $Magento->product_attribute($attribute, $parse);
}

/**
* Added by Todd Lekan www.toddlekan.com 12-10-2012
*/
function magento_product_attribute_get($attribute = 'name', $parse = true){
	global $Magento;
	if($attribute == 'price'){
		
		$return = $Magento->product_price();
	
	} else {
		
		$return = $Magento->product_attribute($attribute, $parse);
	}
	return $return;
}
...
* Added by Todd Lekan www.toddlekan.com 12-10-2012
*/
function magento_products_base64_serialize(){
	global $Magento;
	echo $Magento->products_base64_serialize();
}

/**
* Added by Todd Lekan www.toddlekan.com 12-10-2012
*/
function magento_submit_order($submission){
	
	return Magento::submitOrder($submission);
}