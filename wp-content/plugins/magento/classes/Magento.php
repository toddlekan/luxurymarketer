...
class Magento {
	...
	/**
	 * Added by Todd Lekan www.toddlekan.com 12-10-2012
	 * This function will submit an order
	 *
	 * @param mixed array $submission
	 *
	 * @return String $content
	 */
	public static function submitOrder($submission){
		...
		if($connection){
		    ...
		    try{
				...
				//add cart
				$cartId = $proxy->call( $sessionId, 'cart.create', $store_id );

				//add addresses, see if customer exists
				$call_1 = array(
							array("cart_customer.addresses",
								array(
									$cartId,
									$addresses,
								)
							),
							array('customer.list', array(array('email' => $customer['email'])))
				);

				$customerList = $proxy->multiCall($sessionId, $call_1);

				$call_2 = array();

				//create customer, add addresses
				if(!count($customerList[1])){

					$customer['customer_id'] = $proxy->call($sessionId,'customer.create',array($customer));

					$call_2[] = array('customer_address.create',
									array(
					...
					$call_2[] =	array('customer_address.create',
									array(
										'customerId' => $customerId,
										'addressdata' => $addresses[1]
									)
								);
				} else {

					$customer['customer_id'] = $customerList[1][0]['customer_id'];

					$call_2[] = array('customer.update', array('customerId' => $customer['customer_id'], 'customerData' => $customer));

				}

				//set customer
				$call_2[] = array('cart_customer.set', array( $cartId, $customer));

				//set shipping method
				$call_2[] = array('cart_shipping.method', array($cartId, $shipping_method));

				//add product
				$call_2[] = array('cart_product.add',
					array(
						$cartId,
						$product
					));
				...
				//set payment method
				$call_2[] = array('cart_payment.method',
							array(
								$cartId,
								$payment_method
							)
				...
				//set order
				$call_2[] = array('cart.order',
								array(
									$cartId,
									$customer['store_id'],
									array(
										'GLOBAL_MAGENTO_CC_PAYMENT_METHOD' => $payment_method
									)
								)
							);

				$result = $proxy->multiCall($sessionId, $call_2);

				$e = '';

				foreach($result as $val){
					if($val['faultMessage'] && $val['faultMessage'] != "1" && $val['faultMessage'] != 'Customer not exists.'){
						$e .= $val['faultMessage']."<br />";
					}
				}

				if($e){

					throw new Exception('Error:<br />'.$e);
				}
				...
				//customer check
				$result = $proxy->call($sessionId, 'customer.list', array(array('email' => $customer['email'])));
				...
				if(!count($result)){
					...
					$customer['customer_id'] = $result[0]['customer_id'];

					$result = $proxy->call($sessionId, 'customer.update', array('customerId' => $customer['customer_id'], 'customerData' => $customer));
				...
				$result = $proxy->call($sessionId, 'cart_customer.set', array( $cartId, $customer) );


				//cart_shipping.method => Free
				$result = $proxy->call($sessionId, 'cart_shipping.method', array($cartId, $shipping_method));
				...
				$result = $proxy->call(
					...
					array(
						$cartId,
						$customer['store_id'],
						array('GLOBAL_MAGENTO_CC_PAYMENT_METHOD' => $payment_method)
					)
