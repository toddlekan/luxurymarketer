...
require_once(dirname(__FILE__)."/../../../wp-includes/shortcodes.php");
require_once(dirname(__FILE__)."/../../../wp-includes/widgets.php");
*/
...
function process_email_form() {
	...
	if(isset($_POST['action']) && $_POST['action'] == 'email') {
		...
		if($email_image_verify) {
			...
			if(empty($imageverify)) {
				...
				if($_SESSION['email_verify'] != md5($imageverify)) {
					// not working
					// $error .= '<br /><strong>&raquo;</strong> '.__('Image Verification failed' . $debug, 'wp-email');
				}
			}
		}
		...
		if(empty($error) && not_spamming()) {
			...
			$mail = new PHPMailer();
			$mail->From     = "editor@luxurydaily.com";
